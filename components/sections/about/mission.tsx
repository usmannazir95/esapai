...
export function Mission({
  ...
  useGSAP(() => {
    ...
    // Set initial states for cards based on their position
    cardRefs.current.forEach((card, index) => {
      if (card) {
        // Determine animation direction based on card position
        // First card (left): comes from left
        // Middle card: comes from top
        // Last card (right): comes from right
    ...
    // Animate cards coming from their respective directions
    cardRefs.current.forEach((card, index) => {
      if (card) {
        const isLeftCard = index === 0;
        const isMiddleCard = index === 1;
        ...
        tl.to(
          ...
          isLeftCard || isRightCard ? "-=0.5" : "-=0.3" // Stagger the side cards slightly
        );
      }
    });
